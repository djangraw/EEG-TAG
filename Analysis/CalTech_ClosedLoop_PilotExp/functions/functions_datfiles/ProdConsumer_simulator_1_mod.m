%%%
%%%This version has one extra line of code to change numbering of the
%%%events in the analog events channel in the .dat file.
%%%
...
function [output] = ProdConsumer_simulator_1_mod(Datfile, RT_repository, Classifier)
...
while size(X,2) == NS
    %%%bitand(16348,255)=220%nontarget
    %%%bitand(16338,255)=210%target
    %%%
    %%%Change any events that are of number 160 to 210 (targets)
    %%%Change any events that are of number 80 to 220 (distracters)
    X(1,[bitand(X(1,:),255) == 160]) = 16338;
    X(1,[bitand(X(1,:),255) == 80])  = 16348;
    %%%
    obj = daqdetect(obj,X);
